...
- Communication style must be appropriate to the institution's identity"""
    ...
    def _generate_profile_rule_based(
        ...
        if entity_type_lower in ["student", "alumni"]:
            return {
                "bio": f"{entity_type} besidomintis studijomis ir socialiniais klausimais.",
                "persona": f"{entity_name} yra {entity_type.lower()}, kuris aktyviai dalyvauja akademinėse ir socialinėse diskusijose. Mėgsta dalintis savo nuomone ir bendrauti su bendraamžiais, ypač apie VU Fizikos fakulteto aktualijas.",
                "age": random.randint(18, 25),
                "gender": random.choice(["male", "female"]),
                "mbti": random.choice(self.MBTI_TYPES),
                "country": "Lietuva",
                "profession": "Studentas",
                "interested_topics": ["Studijos", "Technologijos", "Universitetas"],
            }
        
        elif entity_type_lower in ["publicfigure", "expert", "faculty"]:
            return {
                "bio": f"Savo srities ekspertas ir dėstytojas.",
                "persona": f"{entity_name} yra pripažintas {entity_type.lower()}, besidalinantis įžvalgomis apie svarbius klausimus. Žinomas dėl savo patirties ir įtakos viešajame diskurse.",
                "age": random.randint(35, 60),
                "gender": random.choice(["male", "female"]),
                "mbti": random.choice(["ENTJ", "INTJ", "ENTP", "INTP"]),
                "country": "Lietuva",
                "profession": entity_attributes.get("occupation", "Ekspertas"),
                "interested_topics": ["Mokslas", "Švietimas", "Inovacijos"],
            }
        
        elif entity_type_lower in ["mediaoutlet", "socialmediaplatform"]:
            return {
                "bio": f"Oficiali {entity_name} paskyra. Naujienos ir atnaujinimai.",
                "persona": f"{entity_name} yra žiniasklaidos atstovas, pranešantis naujienas. Paskyra dalinasi savalaikiais atnaujinimais ir bendrauja su auditorija.",
                "age": 30,  # 机构虚拟年龄
                "gender": "other",  # 机构使用other
                "mbti": "ISTJ",  # 机构风格：严谨保守
                "country": "Lietuva",
                "profession": "Žiniasklaida",
                "interested_topics": ["Naujienos", "Aktualijos", "Švietimas"],
            }
        
        elif entity_type_lower in ["university", "governmentagency", "ngo", "organization"]:
            return {
                "bio": f"Oficiali {entity_name} paskyra.",
                "persona": f"{entity_name} yra institucija, kuri skelbia oficialias pozicijas, pranešimus ir bendrauja su suinteresuotomis šalimis.",
                "age": 30,  # 机构虚拟年龄
                "gender": "other",  # 机构使用other
                "mbti": "ISTJ",  # 机构风格：严谨保守
                "country": "Lietuva",
                "profession": entity_type,
                "interested_topics": ["Švietimo politika", "Bendruomenė", "Oficialūs pranešimai"],
            }
    ...
    def _save_reddit_json(self, profiles: List[OasisAgentProfile], file_path: str):
        ...
        for idx, profile in enumerate(profiles):
            ...
            item = {
                ...
                "country": profile.country if profile.country else "Lithuania",
            }
